...
def fig_neighborhood_by_brough(sales_NY, brough_name):
    mean_sales_n = (
        sales_NY[sales_NY["Borough"] == brough_name][
    ...
    fig = px.bar(
        ...
        title="Figure: Average price per apartament in each neighborhood of "
        + str(brough_name),
    )
    return fig
...
def fig_neighborhood_by_brough_rent(airbnb_NY, brough_name):
    mean_sales_n = (
        airbnb_NY[airbnb_NY["Borough"] == brough_name][["Neighborhood", "Price"]]
        .groupby("Neighborhood")
        .agg(["mean"])
    )
    mean_sales_n.columns = mean_sales_n.columns.droplevel()
    ...
    fig = px.bar(
        ...
        x="Mean rent price",
        y=mean_sales_n.index,
        title="Figure: Average rent price per apartament in each neighborhood of "
        + str(brough_name),
    )
    return fig
...
with st.sidebar:
    ...
    add_radio = st.radio("Choose borough to see details about it:", (boroughs))

    area = st.slider(
        "What is the gross square feet area you are interested in buying?",
    ...
    min_nights = st.slider(
        "Number of minimum nights to rent a room/apartment:",
        value=[min(airbnb_NY["Minimum Nights"]), max(airbnb_NY["Minimum Nights"])],
        step=1,
    )
    min_rev = st.slider(
        "Number of minimum reviews from renting of a room/apartment:",
        value=[
    ...
    chosen_areas = st.multiselect(
        "Choose neighborhoods for comparison:", all_neighborhoods
    )
...
with tab1:
    with st.expander("Average prices for each borough", expanded=True):
        st.markdown(
            ":red[Mean price per square feet in New York is **"
        ...
        st.markdown(
            ":red[Average price for rent in New York is **"
    ...
    with st.expander(
        "Average prices for each neighborhood in " + str(add_radio), expanded=True
    ):
        for i in boroughs:
            with st.container():
                if add_radio == i:
...
with tab2:
    st.write("""**Here you can compare mean values from chosen neighborhoods:**""")
